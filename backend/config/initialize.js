...
async function initializeDefaultCatalogue() {
	try {
	  ...
	  const defaultCatalogueExists = await catalogueRepository.findOneBy({ name: "Default Catalogue" });
	  ...
	  console.error("Failed to connect to the database or create default catalogue:", error);
	}
  }
