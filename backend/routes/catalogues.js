...
router.get("/", async (req, res) => {
	try {
		...
		res.json(catalogues);
		log.info("Fetched all catalogues.");
	} catch (error) {
		log.error(`Error fetching catalogues: ${error.message}`);
...
router.post("/", async (req, res) => {
	...
	try {
		...
		log.info(`New catalogue created: ${name}`);
	} catch (error) {
		log.error(`Error creating new catalogue: ${error.message}`);
		res
...
router.get("/:id", async (req, res) => {
	try {
		...
		res.json(catalogue);
		log.info(`Fetched catalogue with ID: ${id}`);
	} catch (error) {
		log.error(
...
router.put("/:id", async (req, res) => {
	...
	try {
		...
		log.info(`Updated catalogue with ID: ${id}`);
	} catch (error) {
		log.error(`Error updating catalogue with ID ${id}: ${error.message}`);
		res.status(500).json({ error: error.message });
	}
});
...
router.delete("/:id", async (req, res) => {
	...
	try {
		...
		log.info(`Deleted catalogue with ID: ${id}`);
	} catch (error) {
		log.error(`Error deleting catalogue with ID ${id}: ${error.message}`);
		res.status(500).json({ error: error.message });
	}
});
