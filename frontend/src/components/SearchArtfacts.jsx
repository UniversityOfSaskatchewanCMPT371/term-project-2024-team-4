...
export default function SearchResult({ query, siteId }) {
	...
	const filteredData = data?.filter((item) => item.site.id == siteId);

	// const filteredData = data?.filter((item) =>
	// 	item.site.id.includes(inComingSiteInfo.state.info.id),
	// );
						...
						{filteredData &&
							filteredData.map((item) => (
								<Grid item xs={12} sm={6} md={3} key={item.id}>
									...
									<Card onClick={handleClick2(item)}>
										<CardContent>
