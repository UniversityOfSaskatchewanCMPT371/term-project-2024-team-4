...
describe('CultureModal', () => {
  ...
  it('calls handleSave when save button is clicked', () => {
    const props = {
      ...
      selectedMaterialID: '',
      updateMaterialList: vi.fn(),
      artifactTypes: [{ id: '1', name: 'Artifact Type 1' }, { id: '2', name: 'Artifact Type 2' }],
    };
  ...
  it('alerts the user if no artifact type is selected', () => {
    ...
    const saveButton = getByText('Save');
    fireEvent.click(saveButton);
    expect(window.alert).toHaveBeenCalledWith('Please select an artifact type to proceed.');
  });
  ...
  it('updates material list and closes modal upon successful save', async () => {
    ...
    const mockAxiosPost = vi.spyOn(axios, 'post').mockResolvedValueOnce({ data: { id: '123', name: 'Material 1' } });
