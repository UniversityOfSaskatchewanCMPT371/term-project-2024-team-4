...
describe('CrossSectionModal', () => {
  it('renders correctly', () => {
    ...
    const { getByLabelText, getByText } = render(<CrossSectionModal {...props} />);
  ...
  it('calls handleSave when save button is clicked', async () => {
    ...
    const { getByText, getByLabelText } = render(<CrossSectionModal {...props} />);
    ...
    await waitFor(() => expect(axios.post).toHaveBeenCalledWith('http://localhost:3000/crossSections', { name: 'Test Cross Section' }));
    await waitFor(() => expect(props.updateCrossSectionsList).toHaveBeenCalledWith(mockResponseData.data));
